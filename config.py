...
# dataset_set = {
#     'mscoco': {'training_num': 2000 * 3, 'validation_num': 500 * 3,
#                'data_base_dir_json': 'train2014_json/',
#                'base_dir': 'train2014_vqa_mybuild1/'}
# }
dataset_set = {
    'mscoco': {'training_num': 45000 * 3, 'validation_num': 5000 * 3,
               'data_base_dir_json': 'train2014_json/',
               'base_dir': 'train2014_vqa_mybuild2/'}
}
...
class Config(object):
    def __init__(self, config_name='mcb', dataset_name='mscoco'):
        ...
        data_base_dir = dataset['base_dir'] + 'features/'
        base_dir = dataset['base_dir']
        base_dir_json =  dataset['data_base_dir_json']
        ...
        try:
            ...
            print(self.model_path+' Dir already exists')


        if dataset_name == 'mscoco':
            self.annotations_path = base_dir_json + 'mscoco_train2014_annotations.json'
            self.questions_path = base_dir_json + \
                                  'OpenEnded_mscoco_train2014_questions.json'
            self.val_annotations_path = base_dir_json + 'mscoco_val2014_annotations.json'
            self.val_questions_path = base_dir_json + \
        ...
        self.result_dir = base_dir + 'result_' + config_name

        try:
            os.makedirs(self.log_path)
        except:
            print(self.log_path+' Dir already exists')

        try:
            os.makedirs(self.result_dir)
        except:
            print(self.result_dir+' Dir already exists')
